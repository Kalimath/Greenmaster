...
using ImageConverter = eu.greenmaster.Models.Helpers.ImageConverter;
using StringValidator = eu.greenmaster.Models.Helpers.StringValidator;
...
{
    private static IConfiguration Configuration = new ConfigurationBuilder().SetBasePath(AppContext.BaseDirectory)
    ...
    private static async Task SetImage(Rendering rendering, RenderingViewModel renderingViewModel)
    {
        ...
        if (renderingViewModel.Image != null)
        {
            ...
            var maxHeight = int.Parse(maxHeightConfig as string);
            var maxWidth = int.Parse(maxWidthConfig as string);
